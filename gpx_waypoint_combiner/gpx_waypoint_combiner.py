...
class GPXCombiner:
    ...
    def create_combined_gpx(self, all_waypoints, metadata=None):
        """Create a new GPX object with all waypoints."""
        gpx = gpxpy.gpx.GPX()
        gpx.creator = 'GPX Waypoint Combiner'

        if metadata and any(metadata.values()):
            if metadata.get('name'):
                gpx.name = metadata['name']
            if metadata.get('desc'):
                gpx.description = metadata['desc']
            gpx.time = datetime.now(timezone.utc)

        gpx.waypoints = list(all_waypoints)
        return gpx
...
def collect_input_files(args):
    ...
    return sorted(dict.fromkeys(str(f) for f in input_files))
